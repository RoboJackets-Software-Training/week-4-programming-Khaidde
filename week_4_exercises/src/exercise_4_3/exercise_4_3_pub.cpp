...
int main(int argc, char** argv) {
  ...
  ros::Publisher cos_sin_pub = nh.advertise<week_4_exercises::CustomMessage>("cosine_sine", 1);
  ...
  while(ros::ok()) {
    ...
    week_4_exercises::CustomMessage cos_sin_msg;
    cos_sin_msg.cosine.data = cos(time.toSec());
    cos_sin_msg.sine.data = sin(time.toSec());
    cos_sin_pub.publish(cos_sin_msg);
